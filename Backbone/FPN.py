...
class FPN(nn.Module):
    ...
    def forward(self, input: Dict):
        top_downs = []
        for P_i, lat_conv in zip(input.values(), [self.lat_conv2, self.lat_conv3, self.lat_conv4, self.lat_conv5]):
        ...
        P6 = nn.MaxPool2d(kernel_size = 1, stride = 2)(top_downs[-1])
        output = []
        for out_stage, output_conv in zip(top_downs, [self.output_conv2, self.output_conv3, self.output_conv4, self.output_conv5]):
